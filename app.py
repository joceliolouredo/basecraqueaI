...
def ai_generate_football_quiz(categoria, posicao, tema, qtd):
    ...
    Siga estas regras:
    1. Use linguagem simples, motivadora e jovem.
    2. Misture perguntas teóricas com "Cenários de Jogo" (Ex: 'Você está com a bola...', 'O que você faz nessa jogada?').
    ...
    {{
      "questoes": [
        {{
          ...
          "opcoes": {{"A": "...", "B": "...", "C": "...", "D": "..."}},
          "correta": "A",
          "justificativa": "Explicação curta e educativa."
        }}
      ]
    }}
    """
...
# ==============================================================================
# 4. INICIALIZAÇÃO E INTERFACE (MODERN LIGHT THEME)
# ==============================================================================

# ESSENCIAL: Primeiro inicializa o DB, depois a página
init_db()
...
st.markdown("""
    <style>
    .stApp { background-color: #F8FAF8; color: #2C3E50; }
    [data-testid="stSidebar"] { background-color: #FFFFFF !important; color: #1B5E20 !important; border-right: 1px solid #E0E0E0; }
    .q-card { 
    ...
    .stButton>button:hover { background-color: #FDCB00 !important; transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
    h1, h2, h3 { color: #1B5E20 !important; font-family: 'Segoe UI', Roboto, sans-serif !important; }
    .stTextInput>div>div>input, .stSelectbox>div>div>div, .stNumberInput>div>div>input { 
        background-color: #FFFFFF !important; color: #2C3E20 !important; border: 1px solid #CED4DA !important; 
    }
    </style>
...
if menu == "🏠 Vestiário (Home)":
    ...
    if 'desafio_atual_id' in st.session_state:
        ...
        if not st.session_state.get('desafio_concluido', False):
            ...
            with col_b: st.markdown(f"**Seu Nível:** {nivel}")

            df_stats = pd.DataFrame([{"Área": k, "Perc": (v["corretas"]/v["total"])*100} for k, v in stats.items()])
            fig = go.Figure(data=go.Bar(x=df_stats['Área'], y=df_stats['Perc'], marker_color='#4CAF50'))
    ...
    df = get_desafios()
    if df.empty:
        st.info("Você ainda não participou de nenhum desafio.")
    else:
        opcoes = df['id'].tolist()
